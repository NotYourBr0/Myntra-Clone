...
function MyntraNavbar() {
  // State to track which menu item is hovered
  const [hovered, setHovered] = useState(null);
  // State to store mega menu data from JSON file
  ...
  // Fetch mega menu data from public folder JSON
  useEffect(() => {
    fetch("/data/megaMenu.json")
      .then(res => res.json())
      .then(data => setMenuData(data))
      .catch(err => console.error('Failed to load mega menu:', err));
  }, []);
  ...
  return (
    <nav className="sticky top-0 bg-white shadow-sm z-50">
      <div className="flex items-center justify-between px-4 py-3 md:px-8">
        {/* Myntra Logo */}
        <Link to="/">
          <img src="/images/logo/logo.png" alt="Myntra" className="h-12 w-12 md:h-16 md:w-16" />
        </Link>
          ...
          {menuLabels.map(label => (
            ...
            >
              ...
              {hovered === label && menuData[label] && (
                <div className="absolute left-0 transform pt-4 z-40">
      ...
      {isMobileSearchOpen && (
        <div className="md:hidden fixed inset-0 bg-white bg-opacity-95 z-40 flex flex-col items-center py-8 pt-20">
          ...
          {/* Add popular/recent searches here if desired */}
        </div>
      )}
